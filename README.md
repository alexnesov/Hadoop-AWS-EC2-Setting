...
We create a file called ```bigdata.sh``` in ```/etc/profile.d```, that we transform as an executable through the following command: ```sudo chmod +x /etc/profile.d/bigdata.sh```. 
